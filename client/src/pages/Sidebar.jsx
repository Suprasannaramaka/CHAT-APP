...
console.log("selectedUser:", selectedUser);
  return (
    <div className={`bg-[#25223b] h-full p-5 rounded-xl overflow-hidden overflow-y-scroll text-black ${selectedUser ? "max-md:hidden" : " "}`}>
      <div className="pb-5">
      <div className="flex justify-between items-center">
    <img src={assets.logo} alt="logo" className='max-w-40'/>
     <div className="relative py-2 group">
     <img src={assets.menu_icon} alt="Menu" className='max-h-5 cursor-pointer'/>
     <div className='absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#dbd8e5] border border-gray-900 text-black-100 hidden group-hover:block'>
      ...
      <div className='bg-[#4e4d51] rounded-full flex items-center gap-2 py-3  px-4 mt-5'> 
      ...
      {filteredUsers.map((user , index) => (
        <div  onClick={() =>{setSelectedUser(user) ; setUnseenMessages(prev =>({
         ...
         key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser ?._id === user._id && 'bg-[#282142]/50'} `}>
          ...
          <div className = 'flex flex-col leading-5 text-shadow-white'> 
              <p>{user.fullName}</p>
              {
